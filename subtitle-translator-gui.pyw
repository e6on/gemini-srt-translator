...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                           ...
                           QComboBox, QProgressBar, QGroupBox, QFrame, QGridLayout, )
...
class TranslationWorker(QThread):
    ...
    def __init__(self, input_file, api_key, target_language="Estonian", batch_size=30, 
    ...
    def run(self):
        try:
            ...
            self.error.emit("Counting subtitles in file:", str(self.input_file))
            self.subtitle_count = self._count_subtitles()
            print(f"Found {self.subtitle_count} subtitles")
            self.error.emit("Found " + str(self.subtitle_count) + " subtitles", "")
            
            print(f"Setting up translator with parameters:")         
            print(f"- API Key: {self.api_key[:10]}...")
            print(f"- Target Language: {self.target_language}")
            print(f"- Model Name: {self.model_name}")
            self.error.emit("Setting up translator with parameters:",
             "- API Key: " + str(self.api_key[:10]) + "..." + 
             "\n- Target Language: " + str(self.target_language) + 
             "\n- Model Name: " + str(self.model_name))
            translator = self._setup_translator()
            
            print("Starting translation...")
            self.error.emit("Starting translation...", "")
    ...
    def _count_subtitles(self):
        try:
            with open(self.input_file, 'r', encoding='utf-8') as f:
                ...
                self.error.emit("Successfully counted " + str(count) + " subtitles", "")
                return count
        except Exception as e:
            print(f"Error counting subtitles: {str(e)}")
            self.error.emit("Error counting subtitles:", str(e))
            raise
...
# Subtitle Translator
        ...
        layout.setSpacing(5)
        layout.setColumnMinimumWidth(1, 450)  # Column 1 minimum width
...
# API Key Management
        ...
        api_layout.setContentsMargins(10, 5, 10, 5)
...
# File Management
        ...
        select_files_btn.setFixedWidth(100)
        clear_files_btn.setFixedWidth(100)
...
# Translation Settings
        ...
        self.description_input.setMaximumHeight(30)
        ...
        self.language_combo.addItems([
            "Estonian", "Albanian", "Belarusian", "Bosnian", "Bulgarian", "Catalan", "Croatian", "Czech", "Danish", "Dutch", "English", "Finnish", "French", "Galician", "German", "Greek", "Hungarian", "Icelandic", "Irish", "Italian", "Latvian", "Lithuanian", "Luxembourgish", "Macedonian", "Maltese", "Norwegian", "Polish", "Portuguese", "Romanian", "Russian", "Scots Gaelic", "Serbian", "Slovak", "Slovenian", "Spanish", "Swedish", "Ukrainian", "Welsh", "Yiddish"
        ])
        ...
        layout.addWidget(settings_group, 4, 0)
# Advanced Settings
        ...
        self.batch_input.setPlaceholderText("30")
        self.batch_input.setText("30")
        ...
        layout.addWidget(advanced_group, 5, 0)
# Start Translation
        ...
        layout.addWidget(self.translate_btn, 5, 1)
# Progress
        ...
        self.batches_label = QLabel("Translating progress: --/--")
        self.batches_label.setAlignment(Qt.AlignmentFlag.AlignLeft)
        progress_layout.addWidget(self.batches_label)
        progress_layout.addWidget(self.progress_bar)
        self.time_label = QLabel("Estimated time remaining: --:--")
        self.time_label.setAlignment(Qt.AlignmentFlag.AlignLeft)
        progress_layout.addWidget(self.time_label)
        layout.addWidget(progress_group, 4, 1)
# Logs
        ...
        layout.addWidget(log_group, 2, 1, 2, 1)
        ...
        def update_input_width():
            input_container.setFixedWidth(int(self.width() * 0.25))
        self.resized.connect(update_input_width)
        
        footer_label = QLabel("Made with ❤️ by Amo-Z and modified by e6on")
        footer_label.setLayoutDirection(Qt.LayoutDirection.LeftToRight)
        footer_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(footer_label, 8, 0, 1, 2)
    ...
    def update_progress(self):
        ...
        self.batches_label.setText(f"Translating: {str(self.current_file_batches)}/{str(self.total_file_batches)}")
    ...
    def _validate_translation_params(self):
        try:
            self.batch_size = int(self.batch_input.text() or 30)
